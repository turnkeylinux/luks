...
In order to add a key to the filesystem, we need a currently valid key.

Options:

  --cur-keyfile=    Path to a currently valid filesystem keyfile
  --new-keyfile=    Path to the new keyfile you want to add
...
def key_add(keyadm, cur_keyfile=None, new_keyfile=None):
    cur_key = get_valid_key(keyadm, cur_keyfile)[0]
...
def main():
    ...
    try:
        key_add(keyadm, cur_keyfile, new_keyfile)
